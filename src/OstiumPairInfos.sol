...
contract OstiumPairInfos is IOstiumPairInfos, Initializable {
        ...
        returns (uint256)
    {
        ...
        //      If oiDelta * tradeSize < 0 && leverage < makerMaxLeverage:
        //          - oiDelta * tradeSize < 0 -> trade on less liquidity side
        //          If don't create new imbalance -> makerAmount = |tradeSize|, takerAmount = 0
        //              Just help balance oi so trader get all maker fee
        //              Ex: oiDelta = 100, tradeSize = -50 -> oiDelta + tradeSize = 50 -> oiDelta * (oiDelta + tradeSize) >= 0
        //          Else                          -> makerAmount = |oiDelta|, takerAmount = |oiDelta + tradeSize|
        //              Create new imbalance
        //              Ex: oiDelta = 100, tradeSize = -150 -> oiDelta + tradeSize = -50 -> oiDelta * (oiDelta + tradeSize) < 0
        //      Else:
        //          makerAmount = 0, takerAmount = |tradeSize|
        //   2) return (makerFeeP * makerAmount + takerFeeP * takerAmount) / PRECISION_6 / 100
